...
def set_scheduler(sd_model, sampler_name):
    if (sampler_name == "Euler a"):
        ...
        sd_model.scheduler = DPMSolverSinglestepScheduler.from_config(sd_model.scheduler.config, use_karras_sigmas=True)
    elif (sampler_name == "DPM++ SDE SGM_Uniform"):
        sd_model.scheduler = DPMSolverSinglestepScheduler.from_config(sd_model.scheduler.config, use_karras_sigmas=False, timestep_spacing="trailing")
    elif (sampler_name == "DPM++ SDE Sample SGM_Uniform"):
        sd_model.scheduler = DPMSolverSinglestepScheduler.from_config(sd_model.scheduler.config, use_karras_sigmas=False, timestep_spacing="trailing", prediction_type="sample")
